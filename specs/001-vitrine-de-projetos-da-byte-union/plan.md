...
| Onde mora o portão de cobertura | `angular.json`, em `architect.test.options.coverageThresholds` e `coverageExclude` | `vitest.config.ts`, como este plano dizia antes | **Medido, não suposto**: com o limiar no arquivo de runner, 78 testes verdes reportavam **0% de cobertura** e o portão reprovava por instrumentação, não por falta de teste. O builder `unit-test` gerencia a cobertura e ignora essa parte do runner — como ele já avisa em voz alta para `test.include`. Movido o limiar, a mesma suíte reporta 100% |
| Onde mora a validação de borda | Nos próprios DTOs, em `parseGithubRepositoryDto`, `parseCurationDto` e `parseCatalogDto` | Validar no cliente; validar no caso de uso; confiar na tipagem | Tipagem não sobrevive à fronteira da rede nem à leitura de arquivo: o que chega é `unknown`. O parser confere **forma** e falha ali, com o campo e o tipo recebido na mensagem. Regra **semântica** não entra: resumo vazio, referência inexistente e repositório repetido continuam sendo do `validate_curation_use_case`, como `RF-05` exige |
| Canal de contato como tipo somado | `ContactChannel = ReadyContactChannel \| PendingContactChannel` em `organization_constants` | Texto simples com endereço; texto vazio enquanto não existir | `RF-10` pede dois canais e o Discord ainda não existe. Texto simples só permite duas saídas ruins: endereço inventado, que publica ligação morta na seção que existe para converter interesse em conversa, ou string vazia, que o componente renderiza como canal sem destino. Com o tipo somado, `readyContactChannels()` é o único caminho até o visitante e **renderizar um canal pendente não compila** — enquanto `pendingContactChannels()` mantém a falta visível |
...
| Arquivo | Papel |
|---|---|
| `app/package.json`, `app/angular.json`, `app/tsconfig.json` | Projeto Angular; `sourceRoot` na própria `app/`, para as camadas ficarem na raiz e não sob `src/`. `baseHref: "/"` fixa a raiz do sítio e sustenta `RNF-10`. O `angular.json` carrega também o portão de cobertura. O `tsconfig.json` declara `paths` **sem `baseUrl`**, que o TypeScript 6.0 deprecou e cujo uso falha o build com `TS5101` |
...
| `app/vitest.config.ts` | `runnerConfig` do builder `unit-test`: ambiente, globais e o `include` que o `make it` usa. **Não carrega cobertura** — ver a decisão sobre onde ela mora |
| `app/.prettierignore` | Prettier 3 não lê `.gitignore`; sem este arquivo o `make fmt` reformataria `dist/`, `coverage/` e os artefatos gerados |
